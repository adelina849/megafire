			...
			if(!empty($cek_ses_login))
			{
				...
				//1. GET DATA PELANGGAN/PEMILIK
					$query = "
								...
								LEFT JOIN (SELECT id_pelanggan,COUNT(id_pembelian) AS CNT FROM tb_pembelian GROUP BY id_pelanggan) AS B ON A.id_pelanggan = B.id_pelanggan
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				$query = "
							...
							WHERE MD5(id_pelanggan) = '".htmlentities(str_replace("'","",$_POST['id_pelanggan_md5']), ENT_QUOTES, 'UTF-8')."';
						";
				$this->M_general->exec_query_general($query);
